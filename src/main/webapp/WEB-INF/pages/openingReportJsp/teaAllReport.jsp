                                    ...
                                    <c:otherwise>
                                        <c:forEach var="topic" items="${requestScope.topicList}" varStatus="index">
                                            <tr>
                                                ...
                                                <td>
                                                    <a href="${path}/openingReport/teaToUpdateReport?id=${topic.userCode}">
                                                        <button style="background-color: #3cc8ff; color: #FFFFFF; border: none;font-size: 14px">编辑</button>
                                                    </a> |
                                                    <a href="${path}/openingReport/teaToLookReport?id=${topic.userCode}">
                                                        <button style="background-color: #b2ff1b; color: #FFFFFF; border: none; font-size: 14px">预览</button>
                                                    </a> |
                                                    <a href="${path}/openingReport/teaSaveReport?id=${topic.userCode}" onClick="return confirm('请确保该文件未在本地程序中打开。\n点击确认按钮文件将下载至桌面。名为：学号+姓名+开题报告');" >
