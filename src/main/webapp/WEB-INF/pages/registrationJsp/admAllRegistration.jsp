                    ...
                    <div class="widget">
                        ...
                        <div class="widget-head"  style="position: relative">
                            ...
                            <c:if test="${sessionScope.userType eq 0 }">
                                <div class="row navbar-form " style="position: absolute; top: -5px; right: 50px">
                                    <button type="button" style="margin-right: 10px" onclick="window.location.href='${ctx}/registration/admToAddRegistration';" class="btn btn-success pull-left"><i class="icon-edit"> </i>添加情况表</button>
                                </div>
                            </c:if>
                            <div class="clearfix"></div>
                        </div>
                        ...
                        <div class="widget-content ">
                            ...
                            <table class="table table-striped table-bordered table-hover">
                                ...
                                <tbody id="items">
                                <c:forEach var="registration" items="${requestScope.get('list')}" varStatus="index">
                                    <tr>
                                        <td>${index.count}</td>
                                        <td>${registration.registrationFaculty}</td>
                                        <td>${registration.registrationMajor}</td>
                                        <td>${registration.registrationName}</td>
                                        <td>${registration.registrationNumber}</td>
                                        <td>
                                            <a href="${path}/registration/admToUpdateRegistration?id=${registration.registrationNumber}">
                                                <button style="background-color: #3cc8ff; color: #FFFFFF; border: none;font-size: 14px">编辑</button>
                                            </a> |
                                            <a href="${path}/registration/admToLookRegistration?id=${registration.registrationNumber}">
                                                <button style="background-color: #b2ff1b; color: #FFFFFF; border: none; font-size: 14px">预览</button>
                                            </a> |
                                            <a href="<%=appPath%>/registration/del/${registration.registrationNumber}">
                                                <button style="background-color: #ff0557; color: #FFFFFF; border: none; font-size: 14px">删除</button>
                                            </a>|
                                            <a href="${path}/registration/admSaveRegistration?id=${registration.registrationNumber}" onClick="return confirm('请确保该文件未在本地程序中打开。\n点击确认按钮文件将下载至桌面。名为：学号+姓名+答辩情况登记表');" >
