                        ...
                        <div class="widget-head"  style="position: relative">
                            ...
                            <c:if test="${sessionScope.userType eq 0 }">
                                <div class="row navbar-form " style="position: absolute; top: -5px; right: 50px">
                                    <button type="button" style="margin-right: 10px" onclick="window.location.href='${ctx}/assignment/admToAddAssignment';" class="btn btn-success pull-left"><i class="icon-edit"> </i>添加任务书</button>
                                </div>
                            </c:if>
                            <div class="clearfix"></div>
                        </div>
                            ...
                            <table class="table table-striped table-bordered table-hover">
                                ...
                                <c:forEach var="assignment" items="${requestScope.get('list')}" varStatus="index">
                                    <tr>
                                        ...
                                        <td>
                                            <a href="${path}/assignment/admToUpdateAssignment?id=${assignment.assignmentNumber}">
                                                <button style="background-color: #3cc8ff; color: #FFFFFF; border: none;font-size: 14px">编辑</button>
                                            </a> |
                                            <a href="${path}/assignment/admToLookAssignment?id=${assignment.assignmentNumber}">
                                                <button style="background-color: #b2ff1b; color: #FFFFFF; border: none; font-size: 14px">预览</button>
                                            </a> |
                                            <a href="<%=appPath%>/assignment/del/${assignment.assignmentNumber}">
                                                <button style="background-color: #ff0557; color: #FFFFFF; border: none; font-size: 14px">删除</button>
                                            </a>|
                                            <a href="${path}/assignment/admSaveAssignment?id=${assignment.assignmentNumber}"  onClick="return confirm('请确保该文件未在本地程序中打开。\n点击确认按钮文件将下载至桌面。名为：学号+姓名+任务书');" >
